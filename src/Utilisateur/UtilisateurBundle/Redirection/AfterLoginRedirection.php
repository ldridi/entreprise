 ...
 */
namespace Utilisateur\UtilisateurBundle\Redirection;
...
class AfterLoginRedirection implements AuthenticationSuccessHandlerInterface
{
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token)
    {
        ...
        if (in_array('ROLE_TEST', $rolesTab, true))
            $redirection = new RedirectResponse($this->router->generate('homepage'));
        // otherwise we redirect user to the member area
        else
            $redirection = new RedirectResponse($this->router->generate('entreprise_homepage'));
        return $redirection;
    }
}