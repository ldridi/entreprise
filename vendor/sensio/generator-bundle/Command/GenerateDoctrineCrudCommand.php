...
EOT
    ...
    protected function updateRouting(QuestionHelper $questionHelper, InputInterface $input, OutputInterface $output, BundleInterface $bundle, $format, $entity, $prefix)
    {
        ...
        // first, import the routing file from the bundle's main routingContactsEntreprise.yml file
        $routing = new RoutingManipulator($bundle->getPath().'/Resources/config/routingContactsEntreprise.yml');
        try {
        ...
        if (!$ret) {
            ...
            return array(
                ...
                sprintf('  (%s).', $bundle->getPath().'/Resources/config/routingContactsEntreprise.yml'),
                '',
        ...
        // second, import the bundle's routingContactsEntreprise.yml file from the application's routingContactsEntreprise.yml file
        $routing = new RoutingManipulator($this->getContainer()->getParameter('kernel.root_dir').'/config/routingContactsEntreprise.yml');
        try {
            $ret = $auto ? $routing->addResource($bundle->getName(), 'yml') : false;
        } catch (\RuntimeException $e) {
            // the bundle is already imported form app's routingContactsEntreprise.yml file
            $errorMessage = sprintf(
                "\n\n[ERROR] The bundle's \"Resources/config/routingContactsEntreprise.yml\" file cannot be imported\n".
                "from \"app/config/routingContactsEntreprise.yml\" because the \"%s\" bundle is\n".
        ...
        if (!$ret) {
            return array(
                '- Import the bundle\'s routingContactsEntreprise.yml file in the application routingContactsEntreprise.yml file',
                sprintf('# app/config/routingContactsEntreprise.yml'),
                sprintf('%s:', $bundle->getName()),
                sprintf('    <comment>resource: "@%s/Resources/config/routingContactsEntreprise.yml"</comment>', $bundle->getName()),
                '',
                '# ...',
                '',
            );
    ...
    protected function updateAnnotationRouting(BundleInterface $bundle, $entity, $prefix)
    {
        ...
        $routing = new RoutingManipulator($rootDir.'/config/routingContactsEntreprise.yml');
